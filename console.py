...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        """Create instance specified by a user"""
        if len(line) == 0:
    ...
    def do_show(self, line):
        """Print string representation for 'show' commad: name and id"""
        if len(line) == 0:
            print("** class name missing **")
            return
        args = parse(line)
    ...
    def do_update(self, line):
        ...
        if len(args) >= 4:
            ...
            arg3 = arg3.strip('"')
            arg3 = arg3.strip("'")
            setattr(storage.all()[keys], args[2], cast(arg3))
    ...
    def do_count(self, line):
        """Display the count of instances specified"""
        if line in HBNBCommand.classes:
            count = 0
            for keys, objs in storage.all().items():
                if line in keys:
                    count += 1
            print(count)
        else:
    ...
    def default(self, line):
        """Accepts the class name followed by arguement"""
        args = line.split('.')
        class_arg = args[0]
        if len(args) == 1:
...
def parse(line):
    """Helper method to the parse user typed input"""
